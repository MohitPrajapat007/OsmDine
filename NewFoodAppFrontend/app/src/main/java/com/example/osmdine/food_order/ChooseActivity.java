...
import com.example.osmdine.food_order.Common.Common;
import com.example.osmdine.food_order.Model.User;
import com.example.osmdine.food_order.Server.HomeActivityServer;
import com.example.osmdine.food_order.Server.MainActivityServer;
    ...
    public void onClick(View v) {
        ...
        if (Common.isInternetAvailable(this)) {
            if (v.getId() == R.id.btnClient) {
                ...
                String phone = Paper.book(Common.CLIENT).read(Common.USER_PHONE);
                String password = Paper.book(Common.CLIENT).read(Common.USER_PASSWORD);
                String name = Paper.book(Common.CLIENT).read(Common.USER_NAME);
                ...
                //check if user is already signed in
                String phone = Paper.book(Common.SERVER).read(Common.USER_PHONE);
                String password = Paper.book(Common.SERVER).read(Common.USER_PASSWORD);
                String name = Paper.book(Common.SERVER).read(Common.USER_NAME);
